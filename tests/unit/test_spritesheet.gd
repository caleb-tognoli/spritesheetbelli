...
func test_outline() -> void:
	...
	var round_outline := ImageUtils.outline(img, Color.BLACK, 1)
	assert_eq(round_outline.get_size(), Vector2i(6, 6))
	assert_color(round_outline, Vector2i(2, 2), Color.RED, "inside kept")
	assert_color(round_outline, Vector2i(1, 2), Color.BLACK, "left of it")
	assert_eq(round_outline.get_pixel(1, 1).a, 0.0, "corner left out")
