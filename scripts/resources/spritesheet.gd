...
enum AddMode { SINGLE_SPRITE, ROW }
...
func pad_frames_to_sprite_size():
	for frame_coord in frames:
		if frames[frame_coord].get_size() != sprite_size:
			...
			img.blend_rect(
				frames[frame_coord], center, (img.get_size() - frames[frame_coord].get_size()) / 2
			)
...
func get_image() -> Image:
	var img := Image.create_empty(
		sprite_size.x * grid_size.x, sprite_size.y * grid_size.y, false, Image.Format.FORMAT_RGBA8
	)
	...
	for frame_coord in frames:
		img.blend_rect(
			frames[frame_coord], Rect2i(Vector2i.ZERO, sprite_size), sprite_size * frame_coord
		)
	return img
